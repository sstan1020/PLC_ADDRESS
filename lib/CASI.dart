...
class CGroup_IO {
  ...
  factory CGroup_IO.fromJson(Map<String, dynamic> json) {
    return CGroup_IO(
        vinputs: (json['INPUT'] as List<dynamic>)
            .map((e) => CNode_IO.fromJson(e))
            .toList(),
        voutputs: (json['OUTPUT'] as List<dynamic>)
            .map((e) => CNode_IO.fromJson(e))
            .toList());
  }
}
...
class CSection_IO {
  ...
  factory CSection_IO.fromJson(Map<String, dynamic> json) {
    return CSection_IO(
      vgroups:
          json.map((key, value) => MapEntry(key, CGroup_IO.fromJson(value))),
    );
  }
...
class AddressGenerator {
  final CAddressBase addressBase;

  AddressGenerator({CAddressBase? addressBase})
      : addressBase = addressBase ?? CAddressBase();
...
void main() {
  ...
  for (var entry in vglobalDBNodesPLC17_ASI702B.entries) {
    ...
    if (result.isNotEmpty) {
      ...
      // 使用解析结果生成地址表
      if (vglobalDB_IO.containsKey(result.key)) {
        var addressGen = AddressGenerator();
        var ioGroup = CGroup_IO.fromJson(vglobalDB_IO[result.key]!);
